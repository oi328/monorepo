...
export default function RequestsPage() {
  ...
  const handleSubmitForm = async (e) => {
    ...
    if (!formData.customer_name && !formData.product) {
      alert(isRTL ? 'Ø§Ø³Ù… Ø§Ù„Ø¹Ù…ÙŠÙ„ Ø£Ùˆ Ø§Ù„Ù…Ù†ØªØ¬ Ù…Ø·Ù„ÙˆØ¨' : 'Customer name or product is required')
      return
    }
    setSaving(true)
    try {
      ...
      await load()
      showSuccess(isRTL ? 'ØªÙ… Ø­ÙØ¸ Ø§Ù„Ø·Ù„Ø¨ Ø¨Ù†Ø¬Ø§Ø­' : 'Request saved successfully')
      setShowForm(false)
      setFormData({
        customer_name: '',
        customer_phone: '',
      ...
      alert(isRTL ? 'ÙØ´Ù„ Ø­ÙØ¸ Ø§Ù„Ø·Ù„Ø¨' : 'Failed to save request')
    } finally {
      setSaving(false)
    }
  }
  ...
  const handleApprove = async (id) => {
    if (window.confirm(isRTL ? 'Ù‡Ù„ Ø£Ù†Øª Ù…ØªØ£ÙƒØ¯ Ù…Ù† Ø§Ù„Ù…ÙˆØ§ÙÙ‚Ø© Ø¹Ù„Ù‰ Ù‡Ø°Ø§ Ø§Ù„Ø·Ù„Ø¨ØŸ' : 'Are you sure you want to approve this request?')) {
      try {
        await api.put(`/api/inventory-requests/${id}`, { status: 'Approved' })
        setItems(prev => prev.map(i => i.id === id ? { ...i, status: 'Approved' } : i))
        showSuccess(isRTL ? 'ØªÙ…Øª Ø§Ù„Ù…ÙˆØ§ÙÙ‚Ø© Ø¹Ù„Ù‰ Ø§Ù„Ø·Ù„Ø¨ Ø¨Ù†Ø¬Ø§Ø­' : 'Request approved successfully')
      } catch (e) {
        console.error(e)
  ...
  const handleReject = async (id) => {
    if (window.confirm(isRTL ? 'Ù‡Ù„ Ø£Ù†Øª Ù…ØªØ£ÙƒØ¯ Ù…Ù† Ø±ÙØ¶ Ù‡Ø°Ø§ Ø§Ù„Ø·Ù„Ø¨ØŸ' : 'Are you sure you want to reject this request?')) {
      try {
        await api.put(`/api/inventory-requests/${id}`, { status: 'Rejected' })
        setItems(prev => prev.map(i => i.id === id ? { ...i, status: 'Rejected' } : i))
        showSuccess(isRTL ? 'ØªÙ… Ø±ÙØ¶ Ø§Ù„Ø·Ù„Ø¨ Ø¨Ù†Ø¬Ø§Ø­' : 'Request rejected successfully')
      } catch (e) {
        console.error(e)
  ...
  const handleDelete = async (id) => {
    if (!canManageRequests) {
      alert(isRTL ? 'Ù„Ø§ ØªÙ…Ù„Ùƒ ØµÙ„Ø§Ø­ÙŠØ© Ø­Ø°Ù Ø§Ù„Ø·Ù„Ø¨Ø§Øª' : 'You do not have permission to delete requests')
      return
    }
    if (window.confirm(isRTL ? 'Ù‡Ù„ Ø£Ù†Øª Ù…ØªØ£ÙƒØ¯ Ù…Ù† Ø­Ø°Ù Ù‡Ø°Ø§ Ø§Ù„Ø·Ù„Ø¨ØŸ' : 'Are you sure you want to delete this request?')) {
      try {
        await api.delete(`/api/inventory-requests/${id}`)
        setItems(prev => prev.filter(i => i.id !== id))
        showSuccess(isRTL ? 'ØªÙ… Ø­Ø°Ù Ø§Ù„Ø·Ù„Ø¨ Ø¨Ù†Ø¬Ø§Ø­' : 'Request deleted successfully')
      } catch (e) {
        console.error(e)
  ...
  const handleConvertToQuotation = async (item) => {
    if (window.confirm(isRTL ? 'Ù‡Ù„ ØªØ±ÙŠØ¯ ØªØ­ÙˆÙŠÙ„ Ù‡Ø°Ø§ Ø§Ù„Ø·Ù„Ø¨ Ø¥Ù„Ù‰ Ø¹Ø±Ø¶ Ø³Ø¹Ø±ØŸ' : 'Convert this request to quotation?')) {
      try {
        setLoading(true)
        ...
        try {
          ...
          if (existing) {
            ...
            if (!phone) {
              const promptValue = window.prompt(
                isRTL ? 'Ø±Ù‚Ù… Ù‡Ø§ØªÙ Ø§Ù„Ø¹Ù…ÙŠÙ„ Ù…Ø·Ù„ÙˆØ¨ Ù„Ø¥Ù†Ø´Ø§Ø¡ Ø¹Ù…ÙŠÙ„. Ø§Ø¯Ø®Ù„ Ø±Ù‚Ù… Ø§Ù„Ù‡Ø§ØªÙ:' : 'Customer phone is required to create a customer. Enter phone:'
              )
              phone = String(promptValue || '').trim()
            }
            if (!phone) {
              alert(isRTL ? 'ØªÙ… Ø¥Ù„ØºØ§Ø¡ Ø§Ù„ØªØ­ÙˆÙŠÙ„ Ù„Ø£Ù† Ø±Ù‚Ù… Ø§Ù„Ù‡Ø§ØªÙ ØºÙŠØ± Ù…ØªÙˆÙØ±' : 'Conversion canceled because phone is missing')
              setLoading(false)
              return
            }
          ...
          const msg = hasValidationErrors
            ? (isRTL ? 'ÙØ´Ù„ Ø¥Ù†Ø´Ø§Ø¡/Ø¬Ù„Ø¨ Ø§Ù„Ø¹Ù…ÙŠÙ„ (Ø¨ÙŠØ§Ù†Ø§Øª ØºÙŠØ± Ù…ÙƒØªÙ…Ù„Ø©)' : 'Failed to find/create customer (invalid data)')
            : (isRTL ? 'ÙØ´Ù„ Ø¥Ù†Ø´Ø§Ø¡/Ø¬Ù„Ø¨ Ø§Ù„Ø¹Ù…ÙŠÙ„' : 'Failed to find or create customer')
          alert(msg)
          setLoading(false)
          return
        }
        ...
        showSuccess(isRTL ? 'ØªÙ… Ø§Ù„ØªØ­ÙˆÙŠÙ„ Ø¥Ù„Ù‰ Ø¹Ø±Ø¶ Ø³Ø¹Ø± Ø¨Ù†Ø¬Ø§Ø­' : 'Converted to Quotation successfully')
      } catch (e) {
        console.error('Failed to convert to quotation', e)
        const msg =
          e?.response?.data?.message ||
          (e?.response?.data?.errors ? JSON.stringify(e.response.data.errors) : null) ||
          (isRTL ? 'ÙØ´Ù„ Ø§Ù„ØªØ­ÙˆÙŠÙ„ Ø¥Ù„Ù‰ Ø¹Ø±Ø¶ Ø³Ø¹Ø±' : 'Failed to convert to quotation')
        alert(msg)
      } finally {
        setLoading(false)
      }
  ...
  const handleImport = async (rows) => {
    ...
    for (const row of rows) {
      try {
        const price = Number(row['Price'] || row['Ø§Ù„Ø³Ø¹Ø±']) || 0
        const quantity = Number(row['Quantity'] || row['Ø§Ù„ÙƒÙ…ÙŠØ©']) || 1
        
        const payload = {
          customer_name: row['Customer Name'] || row['Ø§Ø³Ù… Ø§Ù„Ø¹Ù…ÙŠÙ„'],
          customer_phone: row['Customer Phone'] || row['Ø±Ù‚Ù… Ø§Ù„Ù‡Ø§ØªÙ'],
          product: row['Product'] || row['Ø§Ù„Ù…Ù†ØªØ¬'],
          quantity: quantity,
          price: price,
          priority: row['Priority'] || row['Ø§Ù„Ø£ÙˆÙ„ÙˆÙŠØ©'] || 'Medium',
          type: row['Type'] || row['Ù†ÙˆØ¹ Ø§Ù„Ø·Ù„Ø¨'] || 'Inquiry',
          payment_plan: row['Payment Plan'] || row['Ø®Ø·Ø© Ø§Ù„Ø¯ÙØ¹'] || '',
          description: row['Notes'] || row['Ù…Ù„Ø§Ø­Ø¸Ø§Øª'] || '',
          meta_data: {
             price: price,
    ...
    if (successCount > 0) {
      showSuccess(isRTL ? `ØªÙ… Ø§Ø³ØªÙŠØ±Ø§Ø¯ ${successCount} Ø·Ù„Ø¨ Ø¨Ù†Ø¬Ø§Ø­` : `Successfully imported ${successCount} requests`)
      await load()
    } else {
      alert(isRTL ? 'ÙØ´Ù„ Ø§Ù„Ø§Ø³ØªÙŠØ±Ø§Ø¯' : 'Import failed')
    }
  }
        ...
        <div className="flex flex-wrap lg:flex-row lg:items-center justify-between gap-4">
          <div className="w-full lg:w-auto flex items-center justify-between lg:justify-start gap-3">
            <div className="relative flex flex-col items-start gap-1">
              ...
              <p className="text-sm text-[var(--muted-text)] mt-1">
                {isRTL ? 'Ø¥Ø¯Ø§Ø±Ø© Ø·Ù„Ø¨Ø§Øª Ø§Ù„Ø´Ø±Ø§Ø¡' : 'Manage your order requests'}
              </p>
            </div>
          </div>
          ...
          <div className="w-full lg:w-auto flex flex-wrap lg:flex-row items-stretch lg:items-center gap-2 lg:gap-3">
            ...
            {canManageRequests && (
              ...
              >
                <FaPlus />
                {isRTL ? 'Ø¥Ø¶Ø§ÙØ© Ø·Ù„Ø¨' : 'Add Request'}
              </button>
            )}
          </div>
        </div>
      ...
      <div className="glass-panel p-4 rounded-xl mb-6">
        <div className="flex justify-between items-center mb-3">
          <h2 className={`text-sm font-semibold flex items-center gap-2 ${isLight ? 'text-black' : 'text-white'} `}>
            <Filter className="text-blue-500" size={16} /> {isRTL ? 'ØªØµÙÙŠØ©' : 'Filter'}
          </h2>
          <div className="flex items-center gap-1.5 flex-wrap">
            ...
            >
              {showAllFilters ? (isRTL ? 'Ø¥Ø®ÙØ§Ø¡' : 'Hide') : (isRTL ? 'Ø¹Ø±Ø¶ Ø§Ù„ÙƒÙ„' : 'Show All')}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mt-4">
          ...
          <div className="space-y-1">
            ...
            <input
              ...
              placeholder={isRTL ? 'Ø¨Ø­Ø« ÙÙŠ Ø§Ù„Ø·Ù„Ø¨Ø§Øª...' : 'Search requests...'}
            />
          </div>
          ...
          <div className="space-y-1">
            <label className="text-xs font-medium text-[var(--muted-text)] flex items-center gap-1">
              <User className="text-blue-500" size={10} /> {isRTL ? ' Ø§Ù„Ø¹Ù…ÙŠÙ„ ' : 'customer'}
            </label>
            <SearchableSelect
              options={customerOptions}
              value={filters.customer}
              onChange={(val) => setFilters(prev => ({ ...prev, customer: val }))}
              placeholder={isRTL ? 'Ø§Ø®ØªØ± Ø§Ù„Ø¹Ù…ÙŠÙ„...' : 'Select Lead...'}
              isRTL={isRTL}
            />
          </div>
          ...
          <div className="space-y-1">
            <label className="text-xs font-medium text-[var(--muted-text)] flex items-center gap-1">
              <FaShoppingCart className="text-blue-500" size={10} /> {isRTL ? 'Ø¹Ø¯Ø¯ Ø§Ù„Ø¹Ù†Ø§ØµØ±' : 'No. of Items'}
            </label>
            <div className="flex gap-2">
              <input
                type="number"
                className="input w-full text-xs"
                placeholder={isRTL ? 'Ù…Ù†' : 'Min'}
          ...
          <div className="space-y-1">
            <label className="text-xs font-medium text-[var(--muted-text)] flex items-center gap-1">
              <DollarSign className="text-blue-500" size={10} /> {isRTL ? 'Ø§Ù„Ù…Ø¨Ù„Øº Ø§Ù„Ø¥Ø¬Ù…Ø§Ù„ÙŠ' : 'Total Amount'}
            </label>
            <div className="flex gap-2">
              <input
                type="number"
        ...
        {showAllFilters && (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mt-4 pt-4 border-t border-[var(--card-border)]">
            ...
            <div className="space-y-1">
              <label className="text-xs font-medium text-[var(--muted-text)] flex items-center gap-1">
                <User className="text-blue-500" size={10} /> {isRTL ? 'Ø¨ÙˆØ§Ø³Ø·Ø©' : 'Created By'}
              </label>
              <SearchableSelect
            ...
            <div className="space-y-1">
              <label className="text-xs font-medium text-[var(--muted-text)] flex items-center gap-1">
                <User className="text-blue-500" size={10} /> {isRTL ? 'Ù…Ù†Ø¯ÙˆØ¨ Ø§Ù„Ù…Ø¨ÙŠØ¹Ø§Øª' : 'Sales Person'}
              </label>
              <SearchableSelect
            ...
            <div className="space-y-1">
              <label className="text-xs font-medium text-[var(--muted-text)] flex items-center gap-1">
                <Calendar className="text-blue-500" size={10} /> {isRTL ? 'Ø§Ù„ØªØ§Ø±ÙŠØ® Ù…Ù†' : 'Date From'}
              </label>
              <input
                type="date"
                className="input w-full"
            ...
            <div className="space-y-1">
              <label className="text-xs font-medium text-[var(--muted-text)] flex items-center gap-1">
                <Calendar className="text-blue-500" size={10} /> {isRTL ? 'Ø§Ù„ØªØ§Ø±ÙŠØ® Ø¥Ù„Ù‰' : 'Date To'}
              </label>
              <input
                type="date"
                className="input w-full"
          ...
          <table className="w-full text-sm text-left">
              ...
              <tr>
                ...
                <th className="p-4 cursor-pointer hover:text-blue-600" onClick={() => handleSort('id')}>
                  <div className="flex items-center gap-1">
                    {isRTL ? 'Ø±Ù‚Ù… Ø§Ù„Ø·Ù„Ø¨' : 'Request ID'}
                ...
                <th className="p-4 text-center">{isRTL ? 'Ø§Ù„Ø­Ø§Ù„Ø©' : 'Status'}</th>
                <th className="p-4 text-center">{isRTL ? 'Ø§Ù„Ø¹Ù†Ø§ØµØ±' : 'Items'}</th>
                ...
                <th className="p-4">{isRTL ? 'Ø§Ù„Ù…Ù„Ø§Ø­Ø¸Ø§Øª' : 'Notes'}</th>
                <th className="p-4">{isRTL ? 'Ø¨ÙˆØ§Ø³Ø·Ø©' : 'Created By'}</th>
                <th className="p-4">{isRTL ? 'Ø§Ù„ØªØ§Ø±ÙŠØ®' : 'Date'}</th>
            ...
            <tbody className="divide-y divide-[var(--card-border)]">
              {loading ? (
                ...
                <tr>
                  <td colSpan={11 + dynamicFields.length} className="p-8 text-center text-[var(--muted-text)]">
                    {isRTL ? 'Ù„Ø§ ØªÙˆØ¬Ø¯ Ø·Ù„Ø¨Ø§Øª Ù…Ø·Ø§Ø¨Ù‚Ø©' : 'No matching requests found'}
                  </td>
                </tr>
              ) : (
                  ...
                  <tr key={item.id} className="hover:bg-gray-700/50 dark:hover:bg-gray-800/50 transition-colors group">
                    ...
                    <td className="p-4">
                      <div className="flex items-center justify-end gap-2 relative">
                        ...
                        >
                          <FaEye size={14} />
                          <span className="hidden xl:inline">{isRTL ? 'Ù…Ø¹Ø§ÙŠÙ†Ø©' : 'Preview'}</span>
                        </button>
                        ...
                        {item.status === 'Pending' ? (
                          <>
                            <button
                              ...
                              title={isRTL ? 'Ù…ÙˆØ§ÙÙ‚Ø©' : 'Approve'}
                            >
                              <FaCheck size={14} />
                              <span className="hidden xl:inline">{isRTL ? 'Ù…ÙˆØ§ÙÙ‚Ø©' : 'Approve'}</span>
                            </button>
                            ...
                            >
                              <FaBan size={14} />
                              <span className="hidden xl:inline">{isRTL ? 'Ø±ÙØ¶' : 'Reject'}</span>
                            </button>
                              ...
                              {openMenuId === item.id && createPortal(
                                ...
                                >
                                  ...
                                  >
                                    ...
                                    <span className="font-medium">{isRTL ? 'ØªØ­ÙˆÙŠÙ„ Ù„Ø¹Ø±Ø¶ Ø³Ø¹Ø±' : 'Convert to Quotation'}</span>
                                  </button>
                                  ...
                                  >
                                    <FaTrash size={16} />
                                    <span className="font-medium">{isRTL ? 'Ø­Ø°Ù' : 'Delete'}</span>
                                  </button>
                                </div>,
                                document.body
                              )}
                          ...
                          <>
                            {item.status === 'Approved' && (
                              <button
                                ...
                                title={isRTL ? 'ØªØ­ÙˆÙŠÙ„ Ù„Ø¹Ø±Ø¶ Ø³Ø¹Ø±' : 'Convert to Quotation'}
                              >
                                <FaExchangeAlt size={14} />
                                <span className="hidden xl:inline">{isRTL ? 'ØªØ­ÙˆÙŠÙ„' : 'Convert'}</span>
                              </button>
                            )}
                            ...
                            >
                              <FaTrash size={14} />
                              <span className="hidden xl:inline">{isRTL ? 'Ø­Ø°Ù' : 'Delete'}</span>
                            </button>
                          </>
                        )}
                      </div>
            ...
            paginatedItems.map((item) => (
              <div key={item.id} className="card bg-white dark:bg-gray-800 p-4 rounded-xl border border-[var(--card-border)] shadow-sm space-y-3">
                ...
                <div className="grid grid-cols-2 gap-2 text-sm text-gray-600 dark:text-gray-300">
                  <div className="flex flex-col">
                    <span className="text-xs text-[var(--muted-text)]">{isRTL ? 'Ø§Ù„Ø¥Ø¬Ù…Ø§Ù„ÙŠ' : 'Total'}</span>
                    <span className="font-mono font-medium">{Number(item.total).toLocaleString()} {currencySymbol}</span>
                  </div>
                  <div className="flex flex-col">
                    <span className="text-xs text-[var(--muted-text)]">{isRTL ? 'Ø§Ù„Ø¹Ù†Ø§ØµØ±' : 'Items'}</span>
                    <span>{Array.isArray(item.items) ? item.items.length : 0}</span>
                  </div>
                  <div className="flex flex-col">
                    <span className="text-xs text-[var(--muted-text)]">{isRTL ? 'Ø§Ù„ØªØ§Ø±ÙŠØ®' : 'Date'}</span>
                    <span>{new Date(item.createdAt).toLocaleDateString()}</span>
                  </div>
                  <div className="flex flex-col">
                    <span className="text-xs text-[var(--muted-text)]">{isRTL ? 'Ø¨ÙˆØ§Ø³Ø·Ø©' : 'By'}</span>
                ...
                <div className="flex flex-wrap items-center gap-2 pt-3 border-t border-[var(--card-border)] mt-auto">
                  ...
                  >
                    <FaEye size={12} /> {isRTL ? 'Ù…Ø¹Ø§ÙŠÙ†Ø©' : 'Preview'}
                  </button>
        ...
        filteredItems.length > 0 && (
          <div className="mt-4 flex flex-wrap items-center justify-between rounded-xl p-2 border border-gray-100 dark:border-gray-700  gap-4">
            <div className="text-xs text-theme">
              {isRTL
                ? `Ø¹Ø±Ø¶ ${(currentPage - 1) * itemsPerPage + 1} Ø¥Ù„Ù‰ ${Math.min(currentPage * itemsPerPage, filteredItems.length)} Ù…Ù† ${filteredItems.length} ØµÙ†Ù`
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm whitespace-nowrap text-theme">
                {isRTL
                  ? `Ø§Ù„ØµÙØ­Ø© ${currentPage} Ù…Ù† ${Math.ceil(filteredItems.length / itemsPerPage)}`
            ...
            <div className="flex items-center gap-1">
              <span className="text-xs text-gray-500 dark:text-gray-400 whitespace-nowrap">{isRTL ? 'Ù„ÙƒÙ„ ØµÙØ­Ø©:' : 'Per page:'}</span>
              <select
              ...
              <div className="card w-full max-w-xl mt-10 max-h-[85vh] overflow-y-auto">
                ...
                <form onSubmit={handleSubmitForm} className="p-4 space-y-4">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div className="space-y-1">
                      <label className={`text-xs font-medium ${isLight ? 'text-black' : 'text-white'}`}>
                        {isRTL ? 'Ø±Ù‚Ù… Ø§Ù„Ù‡Ø§ØªÙ' : 'Customer Phone'}
                      </label>
                      <input
                        name="customer_phone"
                    ...
                    <div className="space-y-1">
                      <label className={`text-xs font-medium ${isLight ? 'text-black' : 'text-white'}`}>
                        {isRTL ? 'Ø§Ù„Ù…Ù†ØªØ¬ / Ø§Ù„Ø¨Ù†Ø¯' : 'Product / Item'}
                      </label>
                      <input
                        name="product"
                    ...
                    <div className="space-y-1">
                      ...
                      <select
                        ...
                        className="input w-full"
                      >
                        <option value="Low">{isRTL ? 'Ù…Ù†Ø®ÙØ¶Ø©' : 'Low'}</option>
                        <option value="Medium">{isRTL ? 'Ù…ØªÙˆØ³Ø·Ø©' : 'Medium'}</option>
                        <option value="High">{isRTL ? 'Ù…Ø±ØªÙØ¹Ø©' : 'High'}</option>
                      </select>
                    </div>
                    <div className="space-y-1">
                      <label className={`text-xs font-medium ${isLight ? 'text-black' : 'text-white'}`}>
                        {isRTL ? 'Ù†ÙˆØ¹ Ø§Ù„Ø·Ù„Ø¨' : 'Request Type'}
                      </label>
                      <select
                        name="type"
                        value={formData.type}
                        ...
                        className="input w-full"
                      >
                        <option value="Inquiry">{isRTL ? 'Ø§Ø³ØªØ¹Ù„Ø§Ù…' : 'Inquiry'}</option>
                        <option value="Booking">{isRTL ? 'Ø­Ø¬Ø²' : 'Booking'}</option>
                        <option value="Maintenance">{isRTL ? 'ØµÙŠØ§Ù†Ø©' : 'Maintenance'}</option>
                      </select>
                    </div>
                    <div className="space-y-1">
                      <label className={`text-xs font-medium ${isLight ? 'text-black' : 'text-white'}`}>
                        {isRTL ? 'Ø®Ø·Ø© Ø§Ù„Ø¯ÙØ¹' : 'Payment Plan'}
                      </label>
                      <input
                        name="payment_plan"
                  ...
                  <div className="flex justify-end gap-3 pt-2 border-t border-gray-100 dark:border-gray-700 mt-2">
                    ...
                    <button
                      ...
                      disabled={saving}
                    >
                      {saving ? (isRTL ? 'Ø¬Ø§Ø±ÙŠ Ø§Ù„Ø­ÙØ¸...' : 'Saving...') : (isRTL ? 'Ø­ÙØ¸ Ø§Ù„Ø·Ù„Ø¨' : 'Save Request')}
                    </button>
                  </div>
                </form>
              </div>
