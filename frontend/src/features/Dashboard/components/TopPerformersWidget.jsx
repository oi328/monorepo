...
export default function TopPerformersWidget() {
  ...
  return (
    <div className={`rounded-xl shadow-lg border overflow-hidden h-full flex flex-col ${
            ...
            {data.map((user, index) => (
              <div 
                ...
                className={`flex items-center justify-between p-3 rounded-xl transition-all ${
                  index === 0 
                    ? (isLight ? 'bg-amber-50 border border-amber-100 shadow-sm' : 'bg-amber-900/10 border border-amber-500/20') 
                    : (isLight ? 'bg-gray-50 border border-gray-100 hover:bg-gray-100' : 'bg-gray-800/40 border border-gray-700 hover:bg-gray-800/60')
                }`}
              >
                <div className="flex items-center gap-3">
                  {/* Rank Badge */}
                    ...
                    index === 0 
                      ...
                      : index === 1 
                        ...
                        : index === 2 
                          ...
                          : (isLight ? 'bg-white text-gray-600 border border-gray-200' : 'bg-gray-700 text-gray-300 border border-gray-600')
                  }`}>
                ...
                <div className="flex flex-col items-end">
                  <div className="flex items-center gap-1">
                    <span className={`text-lg font-bold ${
                      index === 0 ? 'text-amber-600 dark:text-amber-400' : (isLight ? 'text-gray-900' : 'text-white')
                    }`}>
                      {user.total_actions}
                    </span>
                    <Star className={`w-4 h-4 ${index === 0 ? 'fill-amber-500 text-amber-500' : 'fill-yellow-400 text-yellow-400'}`} />
                  </div>
