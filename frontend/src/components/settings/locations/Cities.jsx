...
export default function Cities({ cities, onEdit, onDelete, onToggleStatus }) {
    ...
    <div className="space-y-6">
      <div className="glass-panel rounded-2xl p-3 md:p-6 overflow-hidden border border-gray-100 dark:border-gray-800 shadow-sm">
        <div className="flex items-center justify-between mb-6">
          <h3 className="text-lg font-semibold text-theme-text dark:text-white">{t('Cities List')}</h3>
        </div>
          ...
          <table className="w-full text-sm text-left">
            <thead className="bg-gray-50/80 dark:bg-gray-800/80 text-gray-500 font-medium backdrop-blur-sm border-b border-gray-200 dark:border-gray-700">
              <tr>
                ...
                <th className="px-3 py-2 md:px-4 md:py-3 whitespace-nowrap">{t('City Name Eng')}</th>
                <th className="px-3 py-2 md:px-4 md:py-3 whitespace-nowrap">{t('City Name Arabic')}</th>
                <th className="px-3 py-2 md:px-4 md:py-3 whitespace-nowrap">{t('Country')}</th>
            ...
            <tbody className="divide-y divide-gray-200 dark:divide-gray-700 ">
              {cities && cities.map((city, index) => (
                <tr key={city.id} className="hover:bg-blue-900/50 transition-colors">
                  <td className="px-3 py-2 md:px-4 md:py-3 text-theme-text font-medium hidden sm:table-cell">{index + 1}</td>
                  <td className="px-3 py-2 md:px-4 md:py-3 text-theme-text font-medium">{city.nameEn}</td>
                  <td className="px-3 py-2 md:px-4 md:py-3 text-theme-text font-medium">{city.nameAr}</td>
                  <td className="px-3 py-2 md:px-4 md:py-3 text-theme-text font-medium">{city.countryName}</td>
                  <td className="px-3 py-2 md:px-4 md:py-3">
                    <label className="relative inline-flex items-center cursor-pointer">
                      <input 
                        ...
                        checked={city.status}
                        onChange={() => onToggleStatus && onToggleStatus(city.id)}
                      />
                  ...
                  <td className="px-3 py-2 md:px-4 md:py-3 text-right flex items-center justify-end gap-2">
                    ...
                    <button 
                      onClick={() => onDelete && onDelete(city.id)}
        ...
        <div className="md:hidden space-y-4">
          {cities && cities.map((city) => (
            <div key={city.id} className="card dark:bg-gray-800 p-4 rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm space-y-4">
              <div className="flex justify-between items-start">
                <div className="space-y-1">
                  <h4 className="font-semibold text-theme-text dark:text-white">{city.nameEn}</h4>
                  <p className="text-sm text-theme-text">{city.nameAr}</p>
                </div>
              ...
              <div className="text-sm text-theme-text">
                <span className="font-medium text-theme-text">{t('Country')}: </span>
                {city.countryName}
              </div>
